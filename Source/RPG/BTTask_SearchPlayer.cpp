...
EBTNodeResult::Type UBTTask_SearchPlayer::ExecuteTask(UBehaviorTreeComponent & OwnerComp, uint8 * NodeMemory)
{
	...
	if (Enemy)
	{
		EnemyLo = (Enemy->GetActorLocation());
		Enemy->GetCharacterMovement()->MaxWalkSpeed = FMath::RandRange(350.f, 500.f);
	}
	...
	if (BBComp && AICon)
	{
		...
		FVector RotationVector = LastTargetRo.Vector(); //마지막 플레이어의 방향 단위벡터를 구함. (Yaw값)
		...
		FVector PreSearch = FVector(RotationVector.X * Rand, RotationVector.Y * Rand, RotationVector.Z); //방향벡터에 Rand를 곱해서 로컬위치를 구함.
